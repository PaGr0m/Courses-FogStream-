...
def get_station_coordinations(url):
    ...
    with open("ISS_coordinates.txt", "w") as fout:
        fout.write("At the moment the international space station is at:\n"
                   "    latitude = {}\n"
...
def get_football_team(url):
    ...
    with open("Top_5_championships_and_teams.txt", "w") as fout:
        fout.write("Top championships:\n")

    for obj in json_obj:
        if obj["league"] in LEAGUE_LIST:
            with open("Top_5_championships_and_teams.txt", "a") as fout:
                fout.write("Information about {0} league:\n"
                           "    ID: {1}\n"
                           "    Title: {2}\n"
                           "    Year: {3}\n"
                           "    Current tour: {4}\n"
            ...
            with open("Top_5_championships_and_teams.txt", "a") as fout:
                fout.write("    Top 5 team:\n")

            count = 1
            for team, goals in top_teams:
                with open("Top_5_championships_and_teams.txt", "a") as fout:
                    fout.write("        {}. {} (goals: {})\n".format(count, team, goals))
                count += 1
            with open("Top_5_championships_and_teams.txt", "a") as fout:
                fout.write("\n\n")
...
def get_pokemons(url):
    ...
    with open("Pokemon.txt", "w") as fout:
        fout.write("Pokemon:\n")
    ...
    for pokemon in json_obj["results"]:
        ...
        with open("Pokemon.txt", "a") as fout:
            fout.write("{}. {}\n"
                       "    Weight: {}\n"
                       "    Height: {}\n"
                       "    Order: {}\n"
...
def main():
    ...
    URL_POKEMONS = "https://pokeapi.co/api/v2/pokemon/?limit=949"
    ...
    while True:
        ...
        if case == "1":
            try:
                fin = open("input_product_list.txt", "r")
            except FileNotFoundError:
                print("File not found")
                break

            ingredients = fin.readline().split()
